...
API_HEADERS = {"X-AIDA-Key": os.getenv("AIDA_API_KEY", ""), "X-AIDA-Actor": "operator"}
...
def api_call(method, path, error_label, **kwargs):
    ...
    try:
        response = requests.request(method, f"{API_BASE}{path}", headers=API_HEADERS, timeout=kwargs.pop("timeout", 60), **kwargs)
        if response.status_code == 200:
            return response.json()
        st.error(f"{error_label} ({response.status_code}): {response.text}")
    except Exception as e:
...
with st.sidebar:
    ...
    if st.button("Run health checks now", help="Disk, memory, load, failed services and certificates. Opens tickets for new problems."):
...
tickets_tab, audit_tab = st.tabs(["Tickets", "Monitoring & Audit"])
    ...
    with right_pane:
        ...
        if tickets is None:
            ...
            for ticket in tickets:
                ...
                origin = "🤖 Auto-detected · " if ticket.get("source") == "monitor" else ""
