...
#
# Needs: SSH key login to that machine already working (ssh-copy-id user@host), and Python 3.10+ there.
...
ssh "${ssh_opts[@]}" -- "$target" 'python3 -c "import sys; assert sys.version_info >= (3, 10), sys.version" && mkdir -p aida-agent/src' \
  || { echo "Could not log in, or Python 3.10+ is missing there. Test with: ssh -p $port $target" >&2; exit 1; }
...
ssh "${ssh_opts[@]}" -- "$target" \
  ...
  || { echo "Setting up Python there failed. On Ubuntu/Debian run there: sudo apt install python3-venv" >&2; exit 1; }
