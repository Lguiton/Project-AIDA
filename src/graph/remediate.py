...
async def remediate_specialist_node(state: AidaState, llm, tools) -> dict:
    ...
    system_prompt = (
        ...
        "service name), and clear_temp_files (delete the user's own old temp files; pass older_than_days if the user "
        "gives an age). Every tool call is shown to a human operator who must approve it before it runs. "
